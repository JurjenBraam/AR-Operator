...
public class mqttManager : MonoBehaviour
{
	...
	private FactoryStatePanelManager factoryState;
	// Start is called before the first frame update
	void Start()
	{
		failures = new bool[9];
		factoryState = GameObject.Find("Factory state Holder").GetComponent<FactoryStatePanelManager>();
	...
	public void sendMovementData(CustomMovement latestMovement)
    {
		if (!client.IsConnected) { return; }

		byte[] movementData = System.Text.Encoding.UTF8.GetBytes(JsonUtility.ToJson(latestMovement));
		client.Publish("factory/ipad/location", movementData, MqttMsgBase.QOS_LEVEL_AT_LEAST_ONCE, false);
    }
